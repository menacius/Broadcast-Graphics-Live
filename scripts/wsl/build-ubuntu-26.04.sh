...
show_help() {
  cat <<'HELP'
Build Broadcast Graphics Live inside Ubuntu 26.04 on WSL2.
...
Optional arguments:
  ...
  --expected-version VER    Required Ubuntu VERSION_ID (default: 26.04)
...
if [[ $INSTALL_DEPS -eq 1 ]]; then
  ...
  apt-get install -y --no-install-recommends \
    ...
    zip unzip tar gzip file patchelf \
    libobs-dev \
...
if [[ -z "$PLATFORM" ]]; then
  case "$arch" in
    x86_64|amd64) PLATFORM="ubuntu-${EXPECTED_VERSION}-x64" ;;
    aarch64|arm64) PLATFORM="ubuntu-${EXPECTED_VERSION}-arm64" ;;
    *) PLATFORM="ubuntu-${EXPECTED_VERSION}-${arch}" ;;
  esac
fi
...
echo "=== Configuring Ubuntu $EXPECTED_VERSION build ==="
...
echo "=== Ubuntu WSL2 build completed successfully ==="
