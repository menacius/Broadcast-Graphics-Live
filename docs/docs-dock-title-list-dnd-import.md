...
Implemented external drag-and-drop import support for the dock title list. The title list now accepts one or more `.ogspt` files dropped from the OS/file manager and routes them through the existing `TitleDataStore::import_title` pipeline.
...
- The title list shows a lightweight OBS-style highlight while a valid `.ogspt` drag is over it.
